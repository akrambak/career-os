...
published: false
description: "Career-OS — open-source crawler + Claude scorer + outreach drafter that runs my own job search. Build log."
...
> Mirror of the canonical post at [bak-dev.com/blog/career-os-launch](https://bak-dev.com/blog/career-os-launch).
> Flip `published: true` once the cover image and canonical URL are live.
...
Three live scrapers, a SQLite store, a Claude-powered fit scorer, an outreach
drafter, and a CLI that ties it all together.
...
I parse that thread, extract the stack / budget / location / contact from
each comment, score them against my profile with Claude, and surface the
top matches. The signal-to-noise ratio is *radically* better than scraping
LinkedIn.
...
Once a job scores well, the next bottleneck is writing a tailored pitch.
The drafter takes the scorer's verdict — including the `suggested_angle`
field — and turns it into a 180-260 word outreach message. Two prompts:
one for FT cover-letter shape, one for freelance pitch shape. Same model.

The freelance pitch never agrees to <2 weeks or hourly under €60/hr —
that's a hard constraint baked into the system prompt, not a soft
"please prefer."
...
**One:** the project is the portfolio. A recruiter or client asking "do you
actually ship AI agents in production?" doesn't need to take my word — the
repo is right there, with commits, tests, CI, and a CLI that actually runs.

**Two:** the people who want to hire me for freelance AI work are exactly
the people who'd look at a public repo before sending a DM. Closed-source
...
- Email-send integration for the digest + drafts
